...
const visibility  = ref('PUBLIC')  // 'PUBLIC' | 'HIDDEN'
const excerpt     = ref('')
const slug        = ref('')
const slugAutoGen = ref(true)
...
async function publish() {
  ...
  try {
    ...
    error.value = e.response?.data?.message || '출간에 실패했습니다.'
  } finally {
...
<template>
  <div class="h-screen flex flex-col bg-gray-50 overflow-hidden">
    ...
    <header class="h-14 shrink-0 bg-white border-b border-gray-200 flex items-center px-5 gap-4 z-20">
      ...
      <!-- Center: Save status + Draft badge -->
      <div class="flex-1 flex items-center justify-center gap-2.5 min-w-0">
        ...
        <!-- Draft badge -->
        <span class="px-2 py-0.5 text-[10px] font-bold tracking-widest uppercase bg-amber-100 text-amber-600 rounded">
          DRAFT
        </span>
      </div>
        ...
        <section class="px-5 py-4 border-b border-gray-100">
          ...
          <div v-if="tags.length" class="flex flex-wrap gap-1.5 mb-2.5">
            ...
            >
              ...
              <button
                @click="removeTag(i)"
                class="text-blue-400 hover:text-blue-600 cursor-pointer leading-none ml-0.5 text-sm"
              >&times;</button>
            </span>
          </div>
